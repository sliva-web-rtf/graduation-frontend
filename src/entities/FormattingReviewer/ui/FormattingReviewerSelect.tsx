...
import { useGetFormattingReviewersQuery } from '../api';

type FormattingReviewerSelectProps = Omit<BaseAutocompleteProps, 'options'>;

export const FormattingReviewerSelect = forwardRef<HTMLInputElement, FormattingReviewerSelectProps>((props, ref) => {
    const { onChange } = props;
    const [open, setOpen] = useState(false);
    const [query, setQuery] = useState('');
    const [queryValue, setQueryValue] = useState('');
    const { data, isFetching } = useGetFormattingReviewersQuery({ query }, { skip: !open });
