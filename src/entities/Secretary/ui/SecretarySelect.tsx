...
import { useGetSecretariesQuery } from '../api';

type SecretarySelectProps = Omit<BaseAutocompleteProps, 'options'>;

export const SecretarySelect = forwardRef<HTMLInputElement, SecretarySelectProps>(
    (props: SecretarySelectProps, ref) => {
        const { onChange } = props;
        const [open, setOpen] = useState(false);
        const [query, setQuery] = useState('');
        const [queryValue, setQueryValue] = useState('');
        const { data, isFetching } = useGetSecretariesQuery({ query }, { skip: !open });
